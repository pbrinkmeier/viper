...
\title{VIPER: Viper Interactive Prolog Education Runtime\\\vspace*{1cm}Pflichtenheft}
...
Zur Lösung eines Problems nutzt Prolog das Konzept der Unifikation. Dies beschreibt die Suche nach einer passenden Substitution für die vorhandenen Variablen, die alle gestellten Term-Gleichungen erfüllt. Dazu werden verschiedene Belegungen getestet, welche unter Umständen keine gültige Lösung sind. Im Falle ungültiger Lösungen wird zu einem vorherigen Stand zurück gelaufen (\enquote{backtracking}) und die nächste Belegung getestet.
...
Die Visualisierung für einzelne Schritte der Interpretation ist hierbei als gerichteter Graph realisiert. Der genannte Graph entspricht einem Baum mit \enquote{Rückwärts-Kanten}, die das Backtracking veranschaulichen.
