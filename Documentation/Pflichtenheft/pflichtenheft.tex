...
Bei erfolgreicher Verarbeitung einer korrekten Datei durch den Parser erlaubt das Programm die Eingabe von Abfragen in das Eingabefeld. Sollten erneut Änderungen an dem geöffneten Prolog-Programm stattfinden, so wird das Eingabefeld sowie jegliche Interaktion mit der Visualisierung gesperrt bis der veränderte Quelltext erneut vom Parser verarbeitet wurde.
...
Die automatisch eingebundene Standardbibliothek kann über eine Schaltfläche deaktiviert werden. Von der Standardbibliothek zur Verfügung gestellte Regeln werden bis zur erneuten Aktivierung nicht mehr berücksichtigt.
...
\nonFunctionality{Inkorrekte Prolog-Programme sollen nie zu einem Absturz des Programms führen.}{nfc:crashresistance}
...
{Der Nutzer gibt eine Anfrage mit mehreren Lösungen ein und bestätigt die Eingabe.}
...
{Zwischenschritte werden visualisiert. In der Konsole erscheint die Lösung.}
...
\teststep{Der Nutzer hat ein Prolog-Programm ausgewählt, welches das formatierte \texttt{simpsons.pl}-Programm aus dem Anhang enthält.}
{Der Nutzer betätigt die Öffnen-Schaltfläche.}
{Der Editor enthält die formatierte \texttt{simpsons.pl}-Programmdatei analog zum Anhang.}
...
{Eine Lösung wurde gefunden. Es wird \texttt{yes.} in der Konsolein der Konsole ausgegeben.}
