...
\criteriumOptional{Export von generierten Graphen als LaTeX-Code}{crt:latexexport}
...
\criteriumOptional{Export von generierten Graphen als Bilddatei}{crt:imageexport}
...
Beim Betätigen der \enquote{Speichern als} so kann über einen Auswahldialog eine neue Datei angelegt werden.
...
%TODO Verlinken
Ein Backtrackingschritt wird entsprechend in der Visualisierung angezeigt.
...
\teststep{Das Programm wird ausgeführt. Ein Prolog-Programm ist im Editor von Hand eingegeben.}
...
\teststep{Das \texttt{simpsons.pl}-Programm ist im Editorfenster eingegeben worden.}
...
\captionof{figure}{Wurzel der Anfrage \texttt{father(X, bart)}}
\end{minipage}
...
\test{Zoomen innerhalb eines generierten Graphen}{tst:zoom}
\tests{fnc:zoom}
...
\test{Bewegung innerhalb eines generierten Graphen}{tst:move}
\tests{fnc:move}
...
\teststep{Das Beispielprogramm ist im Editor geöffnet und der unten gezeigte Zustand wurde erreicht und visualisiert.}
...
\test{Visualierung fehlgeschlagener Unifikation}{tst:unificationfail}
\tests{fnc:unificationfail}

\teststep{Das \texttt{simpsons.pl}-Beispielprogramm ist im Editor eingegeben und durch den Parser verarbeitet worden. Der Nutzer hat die Abfrage \texttt{father(homer, X)} eingegeben und die Wurzel des Visualisierungsgraphen wird angezeigt.}
{Der Nutzer betätigt die Schaltfläche \enquote{Nächster Schritt}.}
{Eine fehlgeschlagene Unifikation mit \texttt{father(abe, homer)} entsprechend der gezeigten Grafik wird angezeigt.}
...
\test{Visualierung erfolgreicher Unifikation}{tst:unificationsuccess}
\tests{fnc:unificationsuccess}

\teststep{Das \texttt{simpsons.pl}-Beispielprogramm ist im Editor eingegeben und durch den Parser verarbeitet worden. Der Nutzer hat die Abfrage \texttt{father(homer, X)} eingegeben und die Wurzel des Visualisierungsgraphen wird angezeigt.}
{Der Nutzer betätigt die Schaltfläche \enquote{Nächster Schritt} zwei mal.}
{Eine erfolgreiche Unifikation mit \texttt{father(homer, bart)} entsprechend der gezeigten Grafik wird angezeigt.}
...
\enquote{Exportieren als TikZ} lässt den Nutzer eine TeX-Datei mit einer Umsetzung des Graphen als TikZ-Grafik erstellen.
...
Um die Visualisierungsbeispiele besser darzustellen zu können, wurden sie hier ohne die grafische Oberfläche des Programms eingefügt.
...
\captionof{figure}{Wurzel des generierten Graphen}
\end{minipage}
...
Wird ein Cut im Kontext einer Abfrage im Zuge des Backtrackings erneut besucht, schlägt die Abfrage fehl.
Anschaulich könnte man sagen, dass ein Cut alle bisherigen Backtracking-Ziele löscht.
...
\textbf{Eingabefeld}:
Element der grafischen Oberfläche. Das Eingabefeld dient zur Eingabe von Abfragen. Es ist nur dann aktiviert, wenn ein korrektes Prolog-Programm eingegeben und geparst wurde.
